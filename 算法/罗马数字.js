...
// console.log(romanToInt('IMV'))


const romanToInt = (str) => {
    return str.split('').reduce((acc,curr,index,originArr) => {
        if( index === originArr.length-1)  return acc + romanEntity[curr]
        return romanEntity[curr] >= romanEntity[originArr[index+1]]
            ? acc + romanEntity[curr]
            : acc - romanEntity[curr]
    },0)
}  
